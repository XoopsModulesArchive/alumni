...
if( $ref == '' || strpos( $ref , XOOPS_URL.'/modules/system/admin.php' ) === 0 ) {
	...
	// 0.75 to 0.76
	$result = $xoopsDB->query( "SELECT event_tz FROM ".$xoopsDB->prefix("pical_event")." LIMIT 1" ) ;
	if( $result ) {
		list( $event_tz ) = $xoopsDB->fetchRow( $result ) ;
		if( ! strstr( $event_tz , '.' ) ) {
			$xoopsDB->queryF( "ALTER TABLE ".$xoopsDB->prefix("pical_event")." MODIFY event_tz float(2,1) NOT NULL default 0.0, MODIFY server_tz float(2,1) NOT NULL default 0.0, MODIFY poster_tz float(2,1) NOT NULL default 0.0, ADD KEY (admission), ADD KEY (allday), ADD KEY (start), ADD KEY (end), ADD KEY (start_date), ADD KEY (end_date), ADD KEY (dtstamp), ADD KEY (unique_id), ADD KEY (cid), ADD KEY (event_tz), ADD KEY (server_tz), ADD KEY (poster_tz), ADD KEY (uid), ADD KEY (groupid), ADD KEY (class), ADD KEY (rrule_pid), ADD KEY (categories)" ) ;
		}
	}
	...
	// 0.76 to 0.8
	$result = $xoopsDB->query( "SELECT * FROM ".$xoopsDB->prefix("pical_plugins")." LIMIT 1" ) ;
	if( ! $result ) {
		$xoopsDB->queryF( "ALTER TABLE ".$xoopsDB->prefix("pical_cat")." CHANGE cat_extkey1 cat_depth TINYINT unsigned NOT NULL DEFAULT 0, ADD cat_style varchar(255) NOT NULL default '', ADD KEY (pid), ADD KEY (weight), ADD KEY (cat_depth)" ) ;

		$xoopsDB->queryF( "CREATE TABLE ".$xoopsDB->prefix("pical_plugins")." (
	...
	// 0.8RC to 0.8RC2
	$result = $xoopsDB->query( "SELECT pi_weight FROM ".$xoopsDB->prefix("pical_plugins")." LIMIT 1" ) ;
	if( ! $result ) {
		$xoopsDB->queryF( "ALTER TABLE ".$xoopsDB->prefix("pical_plugins")." ADD pi_weight smallint(5) unsigned NOT NULL default 0, ADD KEY (pi_weight)" ) ;
	}
