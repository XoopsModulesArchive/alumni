...
class XoopsTree
{
	...
	{
		$idarray =array();
		$result = $this->db->query("SELECT ".$this->id." FROM ".$this->table." WHERE ".$this->pid."=".mysql_real_escape_string($sel_id)."");
	...
	{
		$sql = "SELECT ".$this->id." FROM ".$this->table." WHERE ".$this->pid."=".mysql_real_escape_string($sel_id)."";
	...
	$sql = "SELECT cid, title FROM ".$this->table." WHERE pid=0";
		$categories = alumni_MygetItemIds('alumni_submit');
if(is_array($categories) && count($categories) > 0) {
	...
	global $mydirname;
		...
		$sql = "SELECT ".$this->pid.", ".$title." FROM ".$this->table." WHERE ".$this->id."=".mysql_real_escape_string($sel_id)."";
		$result = $this->db->query($sql);
		if ( $this->db->getRowsNum($result) == 0 ) {
			return $path;
		}
...
$arrow = "<img src=\"".XOOPS_URL."/modules/$mydirname/images/arrow.gif\" alt=\"&raquo;\" />";
		...
		$path = "&nbsp;<a href='".$funcURL."&amp;".$this->id."=".mysql_real_escape_string($sel_id)."'>&nbsp;".$arrow."&nbsp;&nbsp;".$name."</a>
 ".$path."";
		if ( $parentid == 0 ) {
			return $path;
		}
	...
	{
		$result = $this->db->query("SELECT ".$this->pid." FROM ".$this->table." WHERE ".$this->id."=".mysql_real_escape_string($sel_id)."");
		if ( $this->db->getRowsNum($result) == 0 ) {
			return $path;
		}
	...
	{
		$sql = "SELECT * FROM ".$this->table." WHERE ".$this->pid."=".mysql_real_escape_string($sel_id)."";
...
if(is_array($categories) && count($categories) > 0) {
	...
	require_once( XOOPS_ROOT_PATH."/modules/$mydirname/include/gtickets.php" ) ;
		...
		while ( list($catid, $name, $ordre) = $xoopsDB->fetchRow($result) ) {
			...
			echo "<p><a href=\"category.php?op=AlumniNewCat&amp;cid=".addslashes($catid)."\"><img src=\"".XOOPS_URL."/modules/$mydirname/images/plus.gif\" border=0 width=10 height=10 alt=\"".constant($cloned_lang."_ADDSUBCAT")."\"></a>&nbsp;<a href=\"category.php?op=AlumniModCat&amp;cid=".addslashes($catid)."\" title=\"".constant($cloned_lang."_MODIFCAT")."\">$name</a> ";
				if ($xoopsModuleConfig["".$mydirname."_csortorder"] == "ordre") { echo "($ordre)"; }				
				echo "<br />\n";
			$arr = $this->getChildTreeMapArray($catid, $order);
			foreach ( $arr as $option ) {
				$option['prefix'] = str_replace("."," --->",$option['prefix']);
				$catpath = $option['prefix']."&nbsp;<a href=\"category.php?op=AlumniNewCat&amp;cid=".$option[$this->id]."\"><img src=\"".XOOPS_URL."/modules/$mydirname/images/plus.gif\" border=0 width=10 height=10 alt=\"".constant($cloned_lang."_ADDSUBCAT")."\"></a>&nbsp;<a href=\"category.php?op=AlumniModCat&amp;cid=".$option[$this->id]."\" title=\"".constant($cloned_lang."_MODIFCAT")."\">".$myts->htmlSpecialChars($option[$title]);
				$ordreS = $option['ordre'];
				echo "$catpath</a> ";
				if ($xoopsModuleConfig["".$mydirname."_csortorder"] == "ordre") { echo "($ordreS)"; }				
				echo "<br />\n";
