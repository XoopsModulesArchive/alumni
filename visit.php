...
$cid = intval($_GET['cid']);
$result = $xoopsDB->query("select scurl from ".$xoopsDB->prefix("alumni_categories")." where cid= ".mysql_real_escape_string($cid)."");
list($scurl) = $xoopsDB->fetchRow($result);
...
if ( $xoopsModuleConfig['frame'] != "" ) {
	...
	echo "<html><head>
		...
		<frame src='myheader.php?url=$scurl&amp;cid=$cid' frame name='xoopshead' scrolling='no' target='main' Noresize>
